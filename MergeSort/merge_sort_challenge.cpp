...
void parallel_merge_sort(int * array, unsigned int left, unsigned int right, unsigned int depth = 0) {
	...
	if (depth >= std::log(std::thread::hardware_concurrency()))
	{
		...
		unsigned int mid = (left + right) / 2; // find the middle point
		bool spawn_left(false);
		if (spawn_left){
			std::thread left_thread = std::thread(parallel_merge_sort, array, left, mid, depth+1); // sort the left half
			parallel_merge_sort(array, mid+1, right, depth+1);
			left_thread.join();
		}
		else{
			parallel_merge_sort(array, left, mid, depth+1);
			std::thread right_thread = std::thread(parallel_merge_sort, array, mid+1, right, depth+1); // sort the right half
			right_thread.join();
		}
		merge(array, left, mid, right); // merge the two sorted halves
	}	
